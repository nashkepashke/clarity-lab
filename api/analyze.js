...
function buildSystemInstruction(lang) {
  ...
  return (
    ...
    "claim or statement from a user, produce a structured breakdown. Write the claim " +
    "field and every other free-text field (confidenceReason, evidenceBasis, steelman, " +
    "strawmanWarning, tension, whatWouldChangeAssessment) in " + languageName + ". The " +
    "three categorical fields (type, assessment, confidence) are matched against a fixed " +
    "schema — always answer those with exactly one of the English option words listed " +
    "below, never translated or altered, regardless of the output language above.\n" +
    "Keep every field brief — a short phrase or one sentence, except where 2-3 sentences " +
    "are explicitly requested below. Never fabricate specific sources, studies, " +
    "statistics, or numbers you cannot verify; when your assessment relies on general " +
    "background knowledge rather than a specific citable source, say so plainly instead " +
    "of inventing one. Never assign a numeric score or percentage anywhere — assessments " +
    "are categorical only. If a claim is too vague, subjective, or unfalsifiable to " +
    "evaluate, say so ('Not enough information' or 'Not empirically assessable') rather " +
    "than guessing.\n" +
  ...
  for (var attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      geminiRes = await fetch(GEMINI_URL, {
        ...
        body: JSON.stringify({
          systemInstruction: { parts: [{ text: buildSystemInstruction(lang) }] },
